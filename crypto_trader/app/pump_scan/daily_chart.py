...
def _daily_emas_on_intraday(
    ...
    *,
    ...
    """EMA 5/50/100/200 с дневки, привязанные к каждой внутридневной свече."""
    closes_all = [b[4] for b in daily_bars[:closed_count]]
    e5 = _align_ema_to_bars(ema_series(closes_all, 5), len(daily_bars), closed_count)
    ...
    return out5, out50, out100, out200


def _ylim_from_candles(
    ax,
    bars: list[tuple],
    *,
    pad_frac: float = 0.06,
) -> None:
    """Y-шкала только по high/low свечей (EMA за пределами окна — ок)."""
    if not bars:
        return
    lo = min(b[3] for b in bars)
    hi = max(b[2] for b in bars)
    if hi <= lo:
        pad = abs(lo) * 0.002 if lo else 1.0
...
def render_m5_chart_with_daily_ema_png(
    ...
    *,
    ...
    """PNG: ровно `bars_count` свечей 5m + EMA 5/50/100/200 с дневки.

    Y-шкала — только по свечам; дневные EMA могут уходить за край окна.
    """
    import time
    ...
    ema5, ema50, ema100, ema200 = _daily_emas_on_intraday(
    ...
    _plot_ema_lines(ax, dates, ema5, color=_THEME["ema5"], label="EMA5 1D")
    ...
    # Важно: после всех линий — иначе autoscale раздует ось под далёкие EMA
    _ylim_from_candles(ax, display)
