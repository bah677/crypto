...
def _augment_metrics_with_watch_state(
    watch,
    metrics: dict[str, object],
) -> tuple[dict[str, object], float | None, str, bool, str | None]:
    out = dict(metrics)
    price = out.get("price")
    prev_hwm = getattr(watch, "high_watermark_price", None)
    try:
        hwm = float(prev_hwm) if prev_hwm is not None else None
    ...
    try:
        ...
        price_f = None
    if hwm is None and price_f is not None:
        hwm = price_f
    if price_f is not None and hwm is not None:
        hwm = max(hwm, price_f)
        out["price_vs_high_watermark_pct"] = (price_f - hwm) / hwm * 100.0
    else:
        out["price_vs_high_watermark_pct"] = None

    phase = classify_squeeze_phase(out)
    out["squeeze_phase"] = phase
    prev_phase = getattr(watch, "current_phase", None)
    changed = bool(prev_phase and prev_phase != phase)
    reason = None
    if changed:
        reason = f"{prev_phase} -> {phase}"
    return out, hwm, phase, changed, reason
...
async def _run_entry_watch_tick_async() -> None:
    ...
    for watch in watches:
        ...
        metrics, high_watermark, phase, phase_changed, phase_change_reason = _augment_metrics_with_watch_state(
            watch, raw_metrics
        )

        plan = normalize_watch_plan(_loads(watch.watch_plan_json))
        ev = evaluate_watch_plan(plan, metrics)
        phase_history_json = None
        if phase_changed:
            phase_history_json = append_phase_history(
                getattr(watch, "phase_history_json", None),
        ...
        async with session_scope() as session:
            await update_entry_watch_check(
                ...
                current_phase=phase,
                phase_history_json=phase_history_json,
            )
            await session.commit()
        ...
        phase_needs_notice = phase_changed and phase in ("squeeze_deep", "at_resistance", "capitulation", "cooled")
        plan_triggered = ev.triggered

        if phase_changed and phase_needs_notice:
            prev_phase = getattr(watch, "current_phase", None)
            phase_text = format_phase_transition_ru(
            ...
            try:
                await _send_message(
                    watch.telegram_chat_id,
                    f"👀 <b>{html.escape(phase_title_ru(phase))}</b> · "
                    f"<code>{html.escape(watch.symbol)}</code>\n"
                    f"{html.escape(phase_text)}",
                    reply_to_message_id=watch.source_message_id
                    if watch.source_chat_id == watch.telegram_chat_id
                    else None,
                )
            except Exception:
                log.exception("entry_watch phase notify #%s", watch.id)

        should_reeval = bool(plan_triggered or phase in ("at_resistance", "capitulation", "entry_ready"))
        if not should_reeval:
            continue
        if watch.last_llm_at and (now - watch.last_llm_at) < llm_cooldown:
            log.debug("entry_watch #%s reeval cooldown", watch.id)
            continue

        reeval = None
        try:
            hist_raw = getattr(watch, "analysis_history_json", None) or "[]"
            try:
            ...
            reeval = await reeval_entry_watch_async(
                ...
                initial_analysis=getattr(watch, "initial_analysis", None) or "",
                initial_entry_timing=getattr(watch, "initial_entry_timing", None)
                or "unknown",
        ...
        if reeval is not None:
            hist_json = append_analysis_history(
                getattr(watch, "analysis_history_json", None),
                at_iso=now.isoformat(),
                entry_ok=bool(reeval.entry_ok),
                timing=reeval.timing,
                text=reeval.text,
                reason_short=reeval.reason_short,
            )
        ...
        async with session_scope() as session:
            await update_entry_watch_check(
                ...
                analysis_history_json=hist_json,
            )
            await session.commit()
        ...
        if reeval is None:
            ...
            msg = (
                f"👀 <b>Условия плана выполнены</b> · <code>{html.escape(watch.symbol)}</code>\n"
                f"Фаза: {html.escape(phase_title_ru(phase))}\n"
                f"План: {html.escape(format_plan_summary(plan))}\n"
        ...
        try:
            await _send_message(
                watch.telegram_chat_id,
                f"👀 <b>Пока рано входить</b> · <code>{html.escape(watch.symbol)}</code>\n"
                f"Фаза: {html.escape(phase_title_ru(phase))}\n"
                f"{html.escape(note)}\n"
                f"📊 {_metrics_line(metrics)}"
                + (
                    f"\n\nПлан скорректирован:\n{html.escape(format_plan_summary(adjust))}"
                    if adjust
                    else ""
                ),
            )
    ...
    *,
    ...
    async with session_scope() as session:
        ...
        if suggestion is not None:
            ...
            initial_analysis = suggestion.analysis_excerpt or ""
            initial_timing = suggestion.entry_timing or "unknown"
    ...
    async with session_scope() as session:
        ...
        row = await create_entry_watch(
            ...
            initial_entry_timing=initial_timing,
        )
