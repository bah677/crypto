...
async def notify_pump_channel(
    ...
    *,
    ...
    """Pump-алерт. Возвращает (chat_id, message_id) для reply (DeepSeek и др.)."""
    s = get_settings()
    try:
        chat_id, msg_id = await _deliver_pump_alert(
            text,
            parse_mode=parse_mode,
            reply_markup=reply_markup,
            photo=photo,
    ...
    if s.pump_alerts_to_private:
        log.debug("Pump alert → private chat %s", s.superadmin_telegram_id)
    elif not s.telegram_pump_channel_ready:
        log.warning(
            "TELEGRAM_ALERTS_CHAT_ID / TELEGRAM_ALERTS_TOPIC_PUMP не заданы — "
            "pump alert уходит в личку суперадмина"
        )
    return chat_id, msg_id
