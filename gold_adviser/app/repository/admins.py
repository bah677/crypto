...
async def list_admins(session: AsyncSession) -> list[AdminRow]:
    r = await session.execute(select(AdminRow).order_by(AdminRow.created_at.asc()))
    return list(r.scalars().all())


async def list_alert_admins(session: AsyncSession) -> list[AdminRow]:
    r = await session.execute(
        select(AdminRow)
        .where(AdminRow.alerts_enabled.is_(True))
        .order_by(AdminRow.created_at.asc())
    )
    return list(r.scalars().all())
